...
public class SmsAuthenticatorFactory implements AuthenticatorFactory {
	...
	public List<ProviderConfigProperty> getConfigProperties() {
		return List.of(
			...
			new ProviderConfigProperty("forceHttpsApiUrl", "Force HTTPS on API URLs", "When enabled (the default), the SMS API URL must use HTTPS so the one-time code and API credentials are never sent over cleartext HTTP; non-HTTPS gateway URLs are rejected (http://localhost is allowed for development). An admin must intentionally disable this to permit plaintext HTTP.", ProviderConfigProperty.BOOLEAN_TYPE, true),
			...
			new ProviderConfigProperty("hideResponsePayload", "Redacted API response log message", "When enabled (the default), the gateway's API response body is not written to the server log. Disable only for debugging; the response body may contain sensitive data.", ProviderConfigProperty.BOOLEAN_TYPE, true),
			new ProviderConfigProperty("maskPhoneNumberInLogs", "Mask phone number in logs", "When enabled (the default), phone numbers written to the server log are reduced to their last 4 digits (e.g. ••••••1234). Disable only for debugging.", ProviderConfigProperty.BOOLEAN_TYPE, true),
			...
			new ProviderConfigProperty("numberTypeFilters", "Valid number type filters", "Restrict enrolment to these phone-number types. Enforced at enrolment whenever set (independent of 'Format phone number'); a number of any other type is rejected. Empty = allow all types. Possible values are: FIXED_LINE, MOBILE, "
			...
			new ProviderConfigProperty("allowedRegions", "Allowed phone number regions", "Restrict enrolment to phone numbers from these regions (ISO 3166-1 alpha-2, e.g. US, CA, DE). Enforced whenever set (independent of 'Format phone number'); a number outside the allowlist - or one that cannot be validated - is rejected at enrolment. Empty = allow all. Note: +1 spans several regions (US, CA, ...) - add each you want to permit.", ProviderConfigProperty.MULTIVALUED_STRING_TYPE, Collections.emptyList()),
			new ProviderConfigProperty(SmsCodeSender.CONF_COOLDOWN, "Resend cooldown (seconds)", "Minimum seconds a user must wait between SMS code sends within one authentication session. Enforced server-side on the resend button AND on page-refresh re-sends. 0 disables the cooldown. Default 60.", ProviderConfigProperty.STRING_TYPE, "60"),
			new ProviderConfigProperty(SmsCodeSender.CONF_MAX_RESEND, "Max additional codes per session", "Maximum number of additional SMS codes (after the first) that may be sent per authentication session, via the resend button or page refreshes. 0 hides the resend button and blocks refresh re-sends. Default 3.", ProviderConfigProperty.STRING_TYPE, "3")
		);
	}
